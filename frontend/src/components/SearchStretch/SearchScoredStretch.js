...
function SearchStretchController() {
	...
	let content;
	const button = (
		<button className="button__back" onClick={backButtonHandler}>
			Cofnij
		</button>
	);
	switch (mode) {
		...
		case 3:
			content = (
				<>
					...
					<section>
						<Header
							title={selectedStartPoint.name}
							subtitle={"Wybierz punkt końcowy:"}
						/>
						<section className="search__conteiner">
							<EndList
								getPointProps={savePointPropsHandler}
								stretches={response}
		...
		case 4:
			content = (
				<>
					...
					<section>
						<Header
							title={selectedStartPoint.name + " - " + selectedEndPoint.name}
						/>
						<section className="search__conteiner">
							<ScoredStretch
								stretches={response}
